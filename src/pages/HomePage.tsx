...
import { MainPanel as Panel, PrimaryButton, QuickMenuCard } from '../components/console/Console';
...
const shortcutDescriptions = ['파일럿 육성 및 관리','보유 기체 확인','주력 기체 개조','파츠 장착 및 강화','다른 지휘관과 대전','대회 참가'];
const notices = [
  ...
  return <div className="command-home">
    <div className="command-primary">
      <Panel title="지휘관 정보" subtitle="COMMANDER" className="commander-panel">
        ...
        <button className="commander-manage console-button" onClick={() => setPage('pilot')}>파일럿 관리 <span aria-hidden="true">›</span></button><div className="commander-exp"><span>EXP</span><div><small>{maxLevel ? 'MAX LEVEL' : `${pilot.exp.toLocaleString()} / ${expNeed.toLocaleString()}`}</small><progress aria-label="지휘관 경험치" value={maxLevel ? 1 : Math.min(pilot.exp, expNeed)} max={maxLevel ? 1 : expNeed}/></div></div>
      </Panel>
      <Panel title="주력 기체" subtitle="MAIN UNIT" className="unit-panel">
        <div className="unit-body"><img className="unit-portrait" src={unit.image} alt={unit.name}/><div className="unit-data"><h3>{unit.name}</h3><dl>{[['HP',stats.hp],['EN',stats.en],['장갑',stats.armor],['운동성',stats.mobility],['이동력',stats.move]].map(([key,value]) => <div key={key}><dt><img className="stat-icon" src={`/ui/icon_${({HP:'hp',EN:'en',장갑:'armor',운동성:'mobility',이동력:'move'} as Record<string,string>)[key]}.svg`} alt=""/>{key}</dt><dd>{value.toLocaleString()}</dd></div>)}</dl><PrimaryButton onClick={() => { sessionStorage.setItem('rwf-selected-unit', unit.id); setPage('hangar'); }}>기체 상세 보기 <span aria-hidden="true">›</span></PrimaryButton></div></div>
      </Panel>
      <Panel title="다음 작전" subtitle="NEXT OPERATION" className="operation-panel">
        <div className="operation-art"><img src={next?.terrain === 'space' ? '/ui/orbital-dock.png' : '/assets/sortie/hangar.jpg'} alt="작전 구역"/><div className="operation-caption"><h3>{next?.title ?? '모든 작전 완료'}</h3><p>{next?.desc ?? '완료한 작전은 작전 목록에서 확인하십시오.'}</p></div></div>
        <div className="operation-footer"><dl><div><dt>권장 전력</dt><dd>{next ? (next.enemyLevel * 1000).toLocaleString() : '—'}</dd></div><div><dt>지형</dt><dd>{next ? terrainName[next.terrain] : '—'}</dd></div><div><dt>작전 타입</dt><dd>스토리 작전</dd></div></dl><PrimaryButton className="sortie-button" onClick={() => setPage('scenario')}>{next ? '작전 준비' : '작전 목록'} <span aria-hidden="true">»</span></PrimaryButton></div>
      </Panel>
    </div>
    <nav className="command-shortcuts" aria-label="빠른 이동">{shortcuts.map(([page,title,icon], i) => <QuickMenuCard key={page} title={title} description={shortcutDescriptions[i]} icon={icon} onClick={() => setPage(page)}/>)}</nav>
    <div className="command-bottom">
      ...
      <Panel title="진행 중인 이벤트" subtitle="EVENT PREVIEW" className="event-panel"><button className="command-event" onClick={() => setNotice(5)} aria-label="전선 돌파 작전 이벤트 예시 보기"><img src="/ui/orbital-dock.png" alt=""/><img className="event-mech" src={unit.image} alt=""/><div><span className="event-status">준비 중</span><h3>전선 돌파 작전</h3><p>다음 전선을 향한 출격을 준비하십시오.</p><time>기간 미정 · 공개 예정</time></div></button><div className="event-pagination" aria-label="이벤트 미리보기 1개"><button disabled aria-label="이전 이벤트">이전</button><span>01 / 01 · 준비 중</span><button disabled aria-label="다음 이벤트">다음</button></div></Panel>
      <Panel title="일일 임무" subtitle="DAILY MISSION" className="daily-panel"><div className="command-missions">{dailyMissions.map((m) => { const value = Math.min(daily.counts[m.id],m.target); const claimed = daily.claimed.includes(m.id); const done = value >= m.target; return <div className={`command-mission ${done ? 'is-done' : ''}`} key={m.id}><div><label htmlFor={`daily-${m.id}`}><img className="mission-icon" src={`/assets/ui/icon_${m.id === 'upgrade' ? 'develop' : m.id === 'pvp' ? 'pvp' : 'sortie'}.svg`} alt=""/>{m.title}<span>{value}/{m.target}</span></label><progress id={`daily-${m.id}`} value={value} max={m.target}/></div><button disabled={!done || claimed} onClick={() => { if (claimDaily(m.id)) setFeedback(`${m.title} 보상을 수령했습니다.`); }} aria-label={`${m.title}: ${claimed ? '수령 완료' : done ? '보상 수령' : '미완료'}`}><strong><img className={`reward-currency ${m.credit ? 'credit' : 'pp'}`} src={`/ui/icon_${m.credit ? 'credit' : 'pp'}.svg`} alt=""/>{m.credit ? `${m.credit.toLocaleString()} Cr` : `${m.pp} PP`}</strong><small>{claimed ? '수령 완료' : done ? '보상 받기' : '진행 중'}</small></button></div>; })}</div></Panel>
    </div>
