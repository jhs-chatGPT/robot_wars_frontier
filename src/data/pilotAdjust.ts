...
const adjustments: Record<PilotType, Partial<PilotStats>> = {
  '리얼계': { melee: -20, ranged: 15, reaction: 20, control: 20, defense: -15, skill: 10, command: -30 },
  '슈퍼계': { melee: 20, ranged: -15, reaction: -15, control: -10, defense: 25, skill: -5, command: 0 },
};

const statKeys: Array<keyof PilotStats> = ['melee', 'ranged', 'reaction', 'control', 'defense', 'skill', 'command'];
...
export function adaptPilotToType(template: PilotTemplate, type: PilotType): PilotTemplate {
  ...
  let diff = 1200 - statKeys.reduce((sum, key) => sum + stats[key], 0);
  const order: Array<keyof PilotStats> = type === '리얼계'
    ? ['ranged', 'reaction', 'control', 'skill', 'command', 'melee', 'defense']
    : ['melee', 'defense', 'command', 'skill', 'reaction', 'control', 'ranged'];
